...
@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4">Edit Pembimbing</h1>
        <form action="{{ route('admin.pembimbings.update', $pembimbing) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" class="border rounded w-full p-2"
                        value="{{ old('name', $pembimbing->user->name) }}">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" class="border rounded w-full p-2"
                        value="{{ old('email', $pembimbing->user->email) }}">
                    @error('email')
                        <div class="text-red-500 text-sm">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="alamat" id="alamat" class="border rounded w-full p-2">{{ old('alamat', $pembimbing->alamat) }}</textarea>
                    @error('alamat')
                        <div class="text-red-500 text-sm">{{ $message }}</div>
                    @enderror
                </div>
